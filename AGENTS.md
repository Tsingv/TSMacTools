...
- The app installs a minimal `NSApp.mainMenu` with an application menu (Quit) and a standard Edit menu (Undo, Redo, Cut, Copy, Paste, Paste and Match Style, Delete, Select All) using responder-chain selectors with `nil` targets. Without a `paste:`/`cut:` item there is no key equivalent routed to the shared field editor, so Settings text fields (API key, model name, tuning notes) silently ignore Cmd+V/Cmd+X even though their right-click menus still work; any new editable field depends on this Edit menu existing.
